...
set all_projects=Kingdom.Roslyn.Compilation.Core
set all_projects=%all_projects%%delim%Kingdom.Roslyn.Compilation.MSBuild
set all_projects=%all_projects%%delim%Kingdom.Roslyn.Compilation.Services
rem MSBuild only projects
set msbuild_projects=Kingdom.Roslyn.Compilation.MSBuild
rem Services only projects
set services_projects=Kingdom.Roslyn.Compilation.Services
...
:add_msbuild_projects
if /i "%1" equ "--microsoft-build" (
    if /i "%projects%" equ "" (
        set projects=%msbuild_projects%
    ) else (
        set projects=%projects%%delim%%msbuild_projects%
    )
    goto :next_arg
)
if /i "%1" equ "--msbuild" (
